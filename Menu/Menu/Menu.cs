...
namespace Menu
{
    public partial class Menu : Form
    {
        ...
        private void Submenu_Click(object sender, EventArgs e)
        {
            if (anim == 0)
            {
                ...
                switch (pr)
                {
                    case 0:
                        px = this.Width + 1;
                        BenchmarkBtn.Location = new Point(px, py + 2);
                        button3.Location = new Point(px, py + 30);
                        ModoOscuro.Location = new Point(px, py + 58);
                        BenchmarkBtn.Show();
                        button3.Show();
                        ModoOscuro.Show();
                        i = px;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            String blanco = $"{local}\\Sources\\Diapositiva1.PNG";
            String negro = $"{local}\\Sources\\Niggamode.PNG";
            Image Blanco = new Bitmap(blanco);
            Image Negro = new Bitmap(negro);
            switch (Modo)
            {
                case 0:
                    this.BackgroundImage = Negro;
                    Modo = 1;
                    break;
                case 1:
                    this.BackgroundImage = Blanco;
                    Modo = 0;
                    break;
            }
        }
        ...
        {
            i -= 10;
            BenchmarkBtn.Location = new Point(i, py + 2);
            button3.Location = new Point(i, py + 30);
            ModoOscuro.Location = new Point(i, py + 58);
            if (i < this.Width - 200)
            {
                anim = 0;
                pr = 1;
